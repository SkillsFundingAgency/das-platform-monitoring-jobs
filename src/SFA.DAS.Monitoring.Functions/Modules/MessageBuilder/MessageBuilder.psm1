...
function Format-MonitoringServiceResponse {
    ...
    Switch ($MonitoringService) {
        ...
        # --- monitoringService type "Platform" are alerts generated by the signal type Metric in App Insights.
        "Platform" {
            $AppInsightsUri = "https://portal.azure.com/#@/resource$($AlertData.essentials.alertTargetIDs)"

            switch -Wildcard ($AlertData.alertContext.condition.allOf.metricName) {

                "*failures*" {
                    $EncodedUri = [System.Uri]::EscapeUriString("$AppInsightsUri/failures")
                    $Response = @{
                        AppInsightsFailures = "<$EncodedUri|:notebook_with_decorative_cover:>"
                    }
                    break
                }

                default {
                    $EncodedUri = [System.Uri]::EscapeUriString("$AppInsightsUri/overview")
                    $Response = @{
                        AppInsights = "<$EncodedUri|:notebook_with_decorative_cover:>"
                    }
                    break
                }
